...
import billyAngel from "../../images/billiAngel.jpg"
...
function ProfilePage() {
    ...
    const handleDelete = () => {
        invoke("delete_anything", { table: "clients", condition: `where id = '${user.id}'` })
            .then((res) => {
                setMessage(res);
    ...
    const getClient = () => {
        invoke("get_all_users", { user_type: "clients", condition: `where id=${ClientId}` })
            .then((res) => {
                setUser(res[0]);
                console.log(res[0]);
